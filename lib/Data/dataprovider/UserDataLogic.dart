...
//  }
  ...
  static Future<List<dynamic>> readUsersGroupByManagerId(String mangerId) async {
    Dio().options.contentType = Headers.jsonContentType;
    var response =
    await Dio().post(nodeJsUrl + '/findusersofgroupbymanagerid', data: {'mangerId': mangerId});
  ...
  static Future<UserDataModel> readOneUserByMobile(String mobile) async {
    ...
    if(test.length==0)
      {
        UserDataModel user= UserDataModel(id: "-1", name: "name", company: "", telephone: "", creationDate: DateTime.now(), enable: true, mangerId: "", paymentStatus: true, applicationVersion: "", gender: true, age: 5, workScope: "workScope", comment: "comment", enableComment: "enableComment", profilePhoto: "profilePhoto");
        return user;
      }
    else{
      UserDataModel user=UserDataModel.fromList(response.data,0);
